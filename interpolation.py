...
model = networks.__dict__[netName](
...
def interpolate(X0,X1,exp):
    global models
    middle = estimate(X0, X1)
    if exp == 1:
        return [middle]
    first_half = interpolate(X0, middle, exp=exp - 1)
    second_half = interpolate(middle, X1, exp=exp - 1)
    return [*first_half, middle, *second_half]


def interpolate_custom(X0,X1,multiplier):
    offsets = [(iii+1)/(multiplier) for iii in range(multiplier-1)]
    swp = [False for iii in range(multiplier-1)]
    a,b = 0, 1
    c = 1.75
    for iii in range(len(offsets)): # fix offsets for better results
        if offsets[iii]>0.5:
            offsets[iii] = 1 - offsets[iii]
            swp[iii]=True
        offsets[iii] = (((a+(offsets[iii]*2*(b-a)))**c)-(a**c))/((b**c)-(a**c)) / 2
        #offsets[iii] = int(round(offsets[iii] *1024)) / 1024 # ComplexDouble error hotfix
    #print('\n',multiplier,'|',offsets,sep='',flush=True)
    return [estimate(X0,X1,offsets=[offsets[iii]],swap_frames=swp[iii]) for iii in range(multiplier-1)]

def estimate(X0,X1,offsets=[0.5],swap_frames=False):
    global model
    model.time_offsets = offsets
    time_offsets = offsets
    if swap_frames:
        X0,X1 = X1,X0
    X0 = torch.from_numpy(np.transpose(X0, (2,0,1)).astype("float32") / 255.0).type(torch.cuda.FloatTensor)
    X1 = torch.from_numpy(np.transpose(X1, (2,0,1)).astype("float32") / 255.0).type(torch.cuda.FloatTensor)
